...
import DeletePlaygroupPopup from "./DeletePlaygroupPopup";
...
const PlaygroupAdminSettings = ({ playgroupId }: { playgroupId: string }) => {
  ...
  const handleDeletePlaygroup = async () => {
    try {
      await fetch(`/api/playgroups/${playgroupId}/delete-group`, {
        method: "DELETE",
        headers: {
    ...
    <div className="text-textcolor">
      <div className="fixed bottom-36 left-0 right-0 h-14 bg-background z-10 flex justify-evenly items-center border-t-2 border-bg3 px-2">
        ...
        <div className="col-start-4 flex flex-row h-10 w-24 justify-center items-center">
          <Image
            ...
            className="w-6 h-6"
          />
          <p className="text-textcolor font-bold text-xl ml-2">{membersList.length}</p>
        </div>
      </div>
      {showMemberSettings && (
      ...
      {showRemovePopup && selectedMember && (
        ...
        />
            ...
            {showDeletePopup && (
        <DeletePlaygroupPopup
          playgroupName={playgroupName}
          onConfirm={handleDeletePlaygroup}
...
export default PlaygroupAdminSettings;